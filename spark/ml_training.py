...
def save_confusion_matrix_svg(matrix, class_labels, output_path):
    ...
    for row_index, row in enumerate(matrix):
        for col_index, value in enumerate(row):
            ...
            lines.append(
                f'<rect x="{x}" y="{y}" width="{cell_size}" height="{cell_size}" fill="{color_for_value(value)}" stroke="#ffffff" stroke-width="2"/>'
            )
            lines.append(
                f'<text x="{x + cell_size / 2}" y="{y + cell_size / 2 + 6}" text-anchor="middle" font-family="Arial, sans-serif" font-size="16" font-weight="700" fill="#0f172a">{value}</text>'
            )
...
def train_one_model(model_name, estimator, train_df, test_df, feature_names):
    pipeline = Pipeline(stages=[estimator]) if estimator.__class__.__name__ == "OneVsRest" else Pipeline(stages=[estimator])
...
def main():
    ...
    if gold_df.rdd.isEmpty():
        raise ValueError(f"No data found in {INPUT_PATH}. Run the feature engineering step first.")
